...
class Node:
	...
	def forward(self, x):
		...
		self.forward_output = af.forward(self.func_type, self.forward_input) 
	
	# backward
	def backward(self, x):
		self.backward_input = x
		self.backward_output = af.backward(self.func_type, self.forward_output, self.backward_input)
